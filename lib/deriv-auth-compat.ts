...
export function normalizeAuthorizeResponse(authorize: any): NormalizedAuthorize {
  ...
  const isLegacyAccount =
    ...
    !authorize.landing_company &&
    !authorize.account_type

  if (isLegacyAccount) {
    authLog.legacy("Detected legacy Deriv account (missing account_category/landing_company/account_type)", loginid)
  }
 ...
 */
export function storeLegacyOAuthTokens(
  accounts: Array<{ id: string; token: string; currency: string }>
): { tokenMap: Record<string, string>; preferredAccount: { id: string; token: string; currency: string } } {
  ...
  const preferredAccount =
    accounts.find((a) => a.id.startsWith("VR")) || accounts[0]
...
export function cleanOAuthUrlParams(style: "legacy" | "modern" | "both" = "both"): void {
  ...
  if (style === "legacy" || style === "both") {
    ...
    if (url.searchParams.has("scope")) { url.searchParams.delete("scope"); changed = true }
  }
