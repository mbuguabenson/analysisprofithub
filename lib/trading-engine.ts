...
export class TradingEngine {
  static analyzeOverUnder(ticks: number[]): MarketAnalysis {
    ...
    const trendingMarket = lastOverCount > firstOverCount ? "over" : lastOverCount < firstOverCount ? "under" : "neutral"
  ...
  static generateSignals(
    ...
    signals.push({
      ...
      entryPoint: overUnder.overPower > overUnder.underPower ? overUnder.overDigits[0]?.digit : overUnder.underDigits[0]?.digit,
