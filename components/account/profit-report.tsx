...
import { 
  RefreshCw, TrendingUp, TrendingDown, Calendar, Cpu, Zap, 
  Activity, Target, ShieldCheck, Wallet, ArrowDownLeft, ArrowUpRight, ArrowRightLeft
} from "lucide-react"
...
export function ProfitReport({ theme = "dark" }: ProfitReportProps) {
  ...
  const fetchStatementData = useCallback(async (dateFrom?: number, dateTo?: number) => {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] overflow-hidden shadow-[0_30px_60px_rgba(0,0,0,0.4)]">
        ...
        <div className="px-8 py-6 border-b border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white/[0.01]">
          <div>
            <span className="text-xs font-black uppercase tracking-widest text-white/70">Ledger Records</span>
            <p className="text-[9px] text-white/30 font-bold uppercase tracking-widest mt-0.5">Filter by transaction category</p>
          </div>
          ...
          <Table>
            <TableHeader className="bg-white/[0.02]">
              <TableRow className="hover:bg-transparent border-white/[0.05] h-14">
                ...
                <TableHead className="text-[10px] font-black uppercase tracking-widest text-white/30">Protocol/Desc</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-widest text-white/30 text-center">Action</TableHead>
                <TableHead className="text-right text-[10px] font-black uppercase tracking-widest text-white/30">Delta ($)</TableHead>
                <TableHead className="text-right text-[10px] font-black uppercase tracking-widest text-white/30 pr-8">Post-Balance ($)</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {isLoading ? (
                ...
                filteredTransactions.map((tx) => (
                  <TableRow key={tx.transaction_id} className="border-white/[0.03] group/row transition-all hover:bg-white/[0.015]">
                    ...
                    <TableCell className="text-center">
                      <span className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border ${getActionBadge(tx.action_type)}`}>
                        {tx.action_type.replace(/_/g, " ")}
                      </span>
                    </TableCell>
                    <TableCell className="text-right">
                      <span className={`text-[11px] font-mono font-bold tabular-nums ${tx.amount > 0 ? "text-emerald-400" : tx.amount < 0 ? "text-rose-400" : "text-white/30"}`}>
                ...
                <TableRow className="hover:bg-transparent">
                  <TableCell colSpan={6} className="h-64 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <h4 className="text-lg font-black text-white/40 uppercase tracking-tight">Ledger History Null</h4>
        ...
        <div className="px-8 py-5 border-t border-white/[0.05] bg-white/[0.01] flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-black text-white/30 uppercase tracking-widest">{filteredTransactions.length} Nodes Rendered</span>
            </div>
          </div>
