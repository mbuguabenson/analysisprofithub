...
export function AdvancedSignalsGenerator({ ticks, theme = "dark", onSignalGenerated }: AdvancedSignalsGeneratorProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Market Assessment */}
      {marketAssessment && (
        <div className={`rounded-xl p-4 border ${theme === "dark" ? "bg-slate-900/50 border-slate-700/50" : "bg-slate-50 border-slate-200"}`}>
          <div className="flex items-center justify-between mb-3">
            <h3 className={`text-sm font-bold uppercase tracking-wider ${theme === "dark" ? "text-white" : "text-gray-900"}`}>
              Market Assessment
            </h3>
          ...
          <div className="mt-2 h-1 bg-slate-700 rounded-full overflow-hidden">
            <div 
              className={`h-full transition-all ${
                marketAssessment.score > 60 ? "bg-green-500" : 
                marketAssessment.score > 45 ? "bg-yellow-500" : 
                "bg-red-500"
              }`}
        ...
        <div className={`rounded-xl p-4 border ${theme === "dark" ? "bg-slate-900/50 border-slate-700/50" : "bg-slate-50 border-slate-200"}`}>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <p className={`text-[10px] font-semibold uppercase opacity-70 ${theme === "dark" ? "text-slate-300" : "text-slate-600"}`}>Signal</p>
                <p className={`text-lg font-bold ${selectedSignal.signal === "BUY" ? "text-green-400" : "text-gray-400"}`}>
                  {selectedSignal.signal}
                </p>
              </div>
              <div>
                <p className={`text-[10px] font-semibold uppercase opacity-70 ${theme === "dark" ? "text-slate-300" : "text-slate-600"}`}>Power</p>
            ...
            {selectedSignal.entryPoint && (
              <div className="p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
                ...
                <p className="text-sm font-mono font-bold text-green-300">Digit {selectedSignal.entryPoint}</p>
              </div>
            )}
            ...
            {selectedSignal.stopLoss && (
              <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
                ...
                <p className="text-sm font-mono font-bold text-red-300">If {selectedSignal.stopLoss} appears</p>
              </div>
            )}
      ...
      {/* Summary */}
      <div className={`rounded-xl p-3 border flex items-center justify-between ${theme === "dark" ? "bg-slate-900/30 border-slate-700/30" : "bg-slate-100 border-slate-300"}`}>
