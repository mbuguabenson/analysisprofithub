...
export function AIFloatingScanner({ recentDigits, theme = "dark", onMinimize, isMinimized = false }: AIFloatingScannerProps) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className={`px-6 py-4 border-b ${theme === "dark" ? "border-slate-700/50" : "border-gray-200"} flex items-center justify-between`}>
      ...
      <div className={`px-6 py-4 border-b ${theme === "dark" ? "border-slate-700/50" : "border-gray-200"}`}>
        <div className={`p-4 rounded-lg ${getSignalColor()} border border-current/30`}>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={`text-2xl font-bold ${consensus.overallSignal === "BUY" ? "text-green-400" : consensus.overallSignal === "SELL" ? "text-red-400" : "text-yellow-400"}`}>
              {consensus.overallSignal}
            </span>
          </div>
          <div className="grid grid-cols-3 gap-2 text-xs">
            <div>
              <p className={`font-semibold ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Confidence</p>
              <p className="text-base font-bold">{consensus.confidence.toFixed(0)}%</p>
            </div>
            <div>
              <p className={`font-semibold ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Support</p>
              <p className="text-base font-bold">{consensus.supportingStrategies}/6</p>
            </div>
            <div>
              <p className={`font-semibold ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Heat</p>
      ...
      {/* Strategy Details */}
      <div className={`px-6 py-4 border-b ${theme === "dark" ? "border-slate-700/50" : "border-gray-200"}`}>
        <p className={`text-xs font-semibold mb-3 uppercase ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
          Strategy Analysis
        </p>
        <div className="space-y-2">
          {consensus.allStrategies.map((strategy) => (
            <div key={strategy.strategy} className={`px-3 py-2 rounded-lg ${getStrategyColor(strategy.strategy)} text-xs flex items-center justify-between`}>
              <span className="font-semibold">{strategy.strategy.replace(/_/g, "/")}</span>
              <span className="font-bold">{strategy.signal} ({strategy.confidence.toFixed(0)}%)</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Dominant Strategy */}
      <div className={`px-6 py-4 ${theme === "dark" ? "bg-gradient-to-r from-purple-900/30 to-blue-900/30" : "bg-gradient-to-r from-purple-100 to-blue-100"}`}>
        <p className={`text-xs font-semibold mb-2 ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
          Strongest Signal
        </p>
        <div className={`flex items-center gap-2 ${consensus.allStrategies[0].signal === "BUY" ? "text-green-400" : "text-red-400"}`}>
