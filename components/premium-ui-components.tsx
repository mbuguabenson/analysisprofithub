...
export function TradeCard({ symbol, direction, stake, pnl, digit, amount, result, time, bot }: TradeCardProps) {
  ...
  const borderColor = isWin ? "border-emerald-500/30 bg-emerald-500/10" : isLoss ? "border-red-500/30 bg-red-500/10" : "border-white/10 bg-white/5"
  return (
    <div className={`rounded-2xl border p-4 flex items-center justify-between ${borderColor}`}>
      <div>
        ...
        <p className="text-lg font-black text-white">{direction || (digit !== undefined ? `Digit ${digit}` : "—")}</p>
      ...
      <div className="text-right">
        ...
        {result && (
          <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${isWin ? "bg-emerald-500/20 text-emerald-400" : isLoss ? "bg-red-500/20 text-red-400" : "bg-white/10 text-white/40"}`}>
            {result}
          </span>
        )}
      </div>
...
export function GlowBadge({ children, color = "blue", size = "md" }: GlowBadgeProps) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border font-black uppercase tracking-widest ${sizeClass} ${glowColors[color]}`}>
      {children}
    </span>
  )
}
