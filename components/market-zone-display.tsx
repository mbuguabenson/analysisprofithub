...
export function MarketZoneDisplay({ overPower, underPower, volatility, trending, theme = "dark" }: MarketZoneDisplayProps) {
  const [zoneAnalysis, setZoneAnalysis] = useState<ZoneAnalysis | null>(null)

  useEffect(() => {
    const analysis = MarketZoneDetector.analyzeAllMarkets(
      overPower,
      underPower,
      volatility,
      trending
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`rounded-xl p-4 border ${
        ...
        <div className="grid grid-cols-3 gap-3 text-center text-xs">
          <div>
            ...
            <p className={`text-[10px] ${theme === "dark" ? "text-slate-400" : "text-slate-600"}`}>Safe Zones</p>
          </div>
          <div>
            <p className={`font-bold ${theme === "dark" ? "text-yellow-400" : "text-yellow-600"}`}>
              {cautionZones.length}
            </p>
            <p className={`text-[10px] ${theme === "dark" ? "text-slate-400" : "text-slate-600"}`}>Caution</p>
          </div>
          <div>
            <p className={`font-bold ${theme === "dark" ? "text-red-400" : "text-red-600"}`}>
              {badZones.length}
            </p>
            <p className={`text-[10px] ${theme === "dark" ? "text-slate-400" : "text-slate-600"}`}>Bad Zones</p>
          </div>
        </div>
        ...
        {zoneAnalysis.suggestedTrade && (
          <div className={`mt-3 p-3 rounded-lg border flex items-start gap-2 ${
            ...
            <TrendingUp className={`w-4 h-4 shrink-0 mt-0.5 ${theme === "dark" ? "text-green-400" : "text-green-600"}`} />
            <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {zoneAnalysis.zones.map((zone, idx) => (
          ...
          >
            ...
            <p className={`text-[10px] leading-tight ${theme === "dark" ? "text-slate-400" : "text-slate-600"}`}>
              {zone.recommendation}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className={`rounded-lg p-3 border ${theme === "dark" ? "bg-green-500/[0.08] border-green-500/30" : "bg-green-50 border-green-200"}`}>
        ...
        <div className={`rounded-lg p-3 border ${theme === "dark" ? "bg-red-500/[0.08] border-red-500/30" : "bg-red-50 border-red-200"}`}>
